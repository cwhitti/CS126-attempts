...
def results(boxes, box_dictionary, guaranteed):

    extra_boundary = len(str(guaranteed)) - 1

    print(" |===================" + ("=" * extra_boundary) + "|")
...
def calc_guaranteed_amount(pigeons, boxes):
    ...
    print()
    print(" [#] You are guaranteed to always have a box with at least")
    print(f" [#] {guaranteed_amount} pigeons in it.")
    print(" [#] This means there will never be a case where there is no box")
    print(f" [#] containing fewer than {guaranteed_amount} pigeons in it.")
    print()
...
def main():
    ...
    if (run_program == True):
        ...
        guaranteed = calc_guaranteed_amount(pigeons, boxes)
        ...
        for index in range(1, repeat_times+1):
            ...
            results(boxes, box_dictionary,guaranteed)
